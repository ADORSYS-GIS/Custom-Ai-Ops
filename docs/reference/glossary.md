...
| **Chat template** | The model-specific format (special tokens, role markers) that turns a conversation into the single token sequence the model actually sees. |
...
| **GQA** | Grouped-query attention — fewer KV heads than attention heads, shrinking the KV cache by design (e.g. Qwen3-0.6B: 16 heads, 8 KV heads). |
...
| **safetensors** | The standard weight file format: raw tensors + a JSON header (shapes/dtypes readable without loading), safe and memory-mappable. |
