...
✖ commit-lint: message is not a valid Conventional Commit.
  ...
  <scope>       : optional, lowercase — a chart name (model-serving-engine),
                  or deps / adr / release …  e.g. feat(model-serving-engine): …
  !             : optional breaking-change marker  e.g. feat(mcp)!: …

  examples:
    feat(model-serving-engine): add vLLM prefix caching support
    fix(ai-model): correct tensor parallel env var
    docs(adr): ADR-0082 — release-please changelog automation
    chore(deps): bump azure/setup-helm to v5
    refactor(model-serving-engine)!: rename the chart   (breaking)
